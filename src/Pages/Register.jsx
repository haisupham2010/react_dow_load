      ...
      const checkemail = async () => {
        ...
        response.then((response) => response.json()).then((data) => {
          ...
          if (data.kq == "0") {
            ...
            const Signup = async () => {
              const url_singn = "https://tuongdoirong.com/api/node.php?f=inst&database=mindx&mytable=login&cot=email,password&gt='" + credential.email + "','" + credential.password + "'";
  ...
  return (
    <div className="d-flex align-items-center justify-content-center bg-dark-subtle" >
      ...
      <div className="container mt-5" >
        ...
        <div className="row bg-white d-flex align-items-center justify-content-center" style={{  width: "100%px" }}>
          {/* bg-primary */}

          <div className="d-flex justify-content-center" style={{height: "100px", width: "auto"}} >
          ...
          <div className="" style={{ width: "1100px", height: "450px" }}>
            <form onSubmit={handleFormSubmit} >
              ...
              {isemailexists && (<h1 className="" style={{ color: "red" }}>Email already exists</h1>)}
              ...
              {ispassincorrect && (<div style={{ color: "red" }}>
                Password incorrect
              </div>)}

              {isregistered && (<div style={{ color: "blue" }}>
                Successfully registered account
              </div>)}

              <button className="mt-4 w-100 btn btn-lg btn-primary bg-dark" style={{ color: "white" }} type="submit">
                Register
              </button>
            </form>
          </div>
